...
namespace Foodcourt.Model
{
    class BillVIEW
    {
        ...
        public DataTable GetBillWithBillNo()
        {
            ...
            string S = "Select BILL_Id,CONVERT(decimal(17,2),BILL_Amount) AS BILL_Amount,BILL_Tax,CONVERT(decimal(17,2),BILL_Total) AS BILL_Total ,(BILL_Discount+Bill_InstantDis) as Dis_TOTAL,CONVERT(varchar ,BILL_InsertDate ,101) AS BILL_InsertDate, CusName, CusMobile  from FCBILLNO WHERE BILL_Status = 'Settled' and BILL_Id = @SelectedBillNo";
        ...
        public DataTable GetBillWithDate()
        {
            ...
            string S = "Select BILL_Id,CONVERT(decimal(17,2),BILL_Amount) AS BILL_Amount,BILL_Tax,CONVERT(decimal(17,2),BILL_Total) AS BILL_Total ,(BILL_Discount+Bill_InstantDis) as Dis_TOTAL,BILL_InsertDate, CusName, CusMobile from FCBILLNO WHERE BILL_Status = 'Settled' and BILL_InsertDate = @SelectedDate";
            DataTable DT = DbFunctions.ExecuteCommand<DataTable>(S, LIST);
            return DT;
        }
